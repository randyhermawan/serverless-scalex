...
class DeployFunc {
  ...
  ConfigureEvent = async (fnName, fnDef, evtDef, apiData) => {
    ...
    const isScale = evtDef.scale || false;
    ...
    if (isHttpProxy && isScale) {
      if (targetIntegration.IntegrationUri !== evtDef.scaleUrl) {
    ...
    if (isHttpProxy && !isScale) {
      const lambdaIntegration = apiData.Integrations.find(
        (v) =>
          ...
          v.IntegrationUri.includes(`function:${fnDef.name}`)
      );
      ...
      console.log(
        `[scalex event] function '${fnName}' scaled down to be served from Lambda`
      );
    ...
    if (isLambdaProxy && isScale) {
      ...
      console.log(
        `[scalex event] function '${fnName}' scaled up to be served from AppRunner`
      );
      ...
      if (unmatchedIds.length > 0) {
        await Promise.all(
          unmatchedIds.map(async (integrationId) => {
            ...
            console.log(
              `[scalex event] obsolete integration '${integrationId}' removed`
            );
